...
var doAjaxRequest = function(options) {
	...
	xhr.onreadystatechange = function (){
			...
			if (status == 200) {
				...
				if(dataType=='json'){
					try{
						...
						error(xhr.responseText,xhr,"é”™è¯¯çš„æ•°æ®,æ— æ³•è½¬æ¢ä¸ºjson");
					}
					return;
				}
				if(dataType=='xml'){
					if (xhr.responseXML && xhr.responseXML.documentElement) {
						...
						error(xhr.responseText,xhr,"æµè§ˆå™¨ä¸æ”¯æŒajaxè¿”å›žxmlå¯¹è±¡");
					}
					return;
				}
				...
				if(dataType=='json'){
					try{
						var json = $.parseJSON(xhr.responseText);
						error(json,xhr,"æœåŠ¡å™¨è¿”å›žé”™è¯¯ä¿¡æ¯");
					} catch(e){
						error(xhr.responseText,xhr,"æœåŠ¡å™¨è¿”å›žé”™è¯¯ä¿¡æ¯");
					}
					return;
				}
				if(dataType=='xml'){
					if (xhr.responseXML && xhr.responseXML.documentElement) {
						error(xhr.responseXML.documentElement,xhr,"æœåŠ¡å™¨è¿”å›žé”™è¯¯ä¿¡æ¯");
					} else {
						error(xhr.responseText,xhr,"æœåŠ¡å™¨è¿”å›žé”™è¯¯ä¿¡æ¯");
					}
					return;
		...
		if( xhr.readyState === 0){
			error(xhr.responseText,xhr,"æœåŠ¡å™¨å¼‚å¸¸");
		}
	};
	...
	if(options.responseType){
		if(xhr.responseType){
			...
			error('',xhr,"å½“å‰æµè§ˆå™¨ä¸æ”¯æŒè®¾ç½®å“åº”ç±»åž‹");
			return null;
		}
	}
	if(options.mimeType){
		if(hasOverrideMimeType){
			...
			error('',xhr,"å½“å‰æµè§ˆå™¨ä¸æ”¯æŒè®¾ç½®mimeType");
			return null;
		}
	}
	...
	for(var key in headers){
		if(hasSetRequestHeader){
			...
			error('',xhr,"å½“å‰æµè§ˆå™¨ä¸æ”¯æŒè®¾ç½®header");
			return null;
		}
	}
...
var uploadFn = function(options) {
	...
	if (!isCanUploadFile) {
		options.onFileUploadError({
			...
			msg : 'å½“å‰æµè§ˆå™¨ä¸æ”¯æŒå¼‚æ­¥ä¸Šä¼ æ–‡ä»¶,è¯·æ¢ç”¨å…¶ä»–æµè§ˆå™¨'
		});
		return;
	}
	...
	if (!acc) {
		options.onFileUploadError({
			...
			msg : 'ç”¨æˆ·æœªç™»å½•åˆ°usergridæœåŠ¡å™¨,æ— æ³•ä½¿ç”¨æ–‡ä»¶ä¸Šä¼ åŠŸèƒ½'
		});
		return;
	}
	...
	if(fileSize > 10485760){
		options.onFileUploadError({
			type : EASEMOB_IM_UPLOADFILE_ERROR,
			msg : 'ä¸Šä¼ æ–‡ä»¶è¶…è¿‡æœåŠ¡å™¨å¤§å°é™åˆ¶ï¼ˆ10Mï¼‰'
		});
		return ;
	}else if(fileSize <= 0){
		options.onFileUploadError({
			type : EASEMOB_IM_UPLOADFILE_ERROR,
			msg : 'ä¸Šä¼ æ–‡ä»¶å¤§å°ä¸º0'
		});
		return ;
	}
	...
	if(xhr.addEventListener){
		...
		xhr.addEventListener("load", function(e) {
			try{
				...
				options.onFileUploadError({
					type : EASEMOB_IM_UPLOADFILE_ERROR,
					msg : 'ä¸Šä¼ æ–‡ä»¶å¤±è´¥,æœåŠ¡ç«¯è¿”å›žå€¼å€¼ä¸æ­£ç¡®',
					e : e,
			...
			if( xhr.readyState === 4){
				if (ajax.status == 200) {
					try{
						...
						options.onFileUploadError({
							type : EASEMOB_IM_UPLOADFILE_ERROR,
							msg : 'ä¸Šä¼ æ–‡ä»¶å¤±è´¥,æœåŠ¡ç«¯è¿”å›žå€¼ä¸æ­£ç¡®',
							e : e,
							data : xhr.responseText,
							xhr : xhr
						});
						...
						options.onFileUploadError({
							type : EASEMOB_IM_UPLOADFILE_ERROR,
							msg : 'ä¸Šä¼ æ–‡ä»¶å¤±è´¥,æœåŠ¡ç«¯è¿”å›žå¼‚å¸¸',
							data : xhr.responseText,
							xhr : xhr
						});
				}
...
var downloadFn = function(options){
	...
	if (!isCanDownLoadFile) {
		options.onFileDownloadError({
			...
			msg : 'å½“å‰æµè§ˆå™¨ä¸æ”¯æŒå¼‚æ­¥ä¸‹è½½æ–‡ä»¶,è¯·æ¢ç”¨å…¶ä»–æµè§ˆå™¨'
		});
		return;
	}
	...
	if (!accessToken) {
		options.onFileDownloadError({
			...
			msg : 'ç”¨æˆ·æœªç™»å½•åˆ°usergridæœåŠ¡å™¨,æ— æ³•ä½¿ç”¨æ–‡ä»¶ä¸‹è½½åŠŸèƒ½'
		});
		return;
	}
			...
			if( xhr.readyState === 4){
				if (ajax.status == 200) {
					options.onFileDownloadComplete(xhr.response,xhr);
						...
						options.onFileDownloadError({
							type : EASEMOB_IM_DOWNLOADFILE_ERROR,
							msg : 'ä¸‹è½½æ–‡ä»¶å¤±è´¥,æœåŠ¡ç«¯è¿”å›žå¼‚å¸¸',
							xhr : xhr
						});
				}
			} else {
...
var login2UserGrid = function(options){
	...
	if(devInfos.length!=2){
		error({
			...
			msg : 'è¯·æŒ‡å®šæ­£ç¡®çš„å¼€å‘è€…ä¿¡æ¯(appKey)'
		});
		return false;
	}
...
var innerCheck = function(options,conn){
	if (conn.isOpened() || conn.isOpening()) {
		conn.onError({
			...
			msg : 'é‡å¤æ‰“å¼€è¿žæŽ¥,è¯·å…ˆå…³é—­è¿žæŽ¥å†æ‰“å¼€'
		});
		return false;
	}
	options = options || {};
	...
	if (status == Strophe.Status.CONNFAIL){
	  conn.onError({
		...
		conn.onError({
			...
			msg : 'ç™»å½•å¤±è´¥,è¯·è¾“å…¥æ­£ç¡®çš„ç”¨æˆ·åå’Œå¯†ç '
		});
		conn.clear();
	...
	if(options.accessToken){
		...
		var error = function(res,xhr,msg){
			if(res.error && res.error_description){
				conn.onError({
					type : EASEMOB_IM_CONNCTION_OPEN_USERGRID_ERROR,
					msg : "ç™»å½•å¤±è´¥,"+res.error_description,
					data : res,
					xhr : xhr
				});
			} else {
...
connection.prototype.attach = function(options) {
	...
	if(accessToken == ''){
		this.onError({
			...
			msg : 'æœªæŒ‡å®šç”¨æˆ·çš„accessToken'
		});
		return;
	}
...
connection.prototype.sendCommand = function(dom) {
	if(this.isOpened()){
		...
		this.onError({
			type : EASEMOB_IM_CONNCTION_OPEN_ERROR,
			msg : 'è¿žæŽ¥è¿˜æœªå»ºç«‹,è¯·å…ˆç™»å½•æˆ–ç­‰å¾…ç™»å½•å¤„ç†å®Œæ¯•'
		});
	}
};
...
connection.prototype.sendPicture = function(options) {
	...
	image.onerror = function() {
		image.onerror = function(){
			...
			options.onFileUploadError({
				type : EASEMOB_IM_UPLOADFILE_ERROR,
				msg : 'æŒ‡å®šçš„å›¾ç‰‡ä¸å­˜åœ¨æˆ–è€…ä¸æ˜¯ä¸€ä¸ªå›¾ç‰‡æ ¼å¼æ–‡ä»¶'
			});
		};
