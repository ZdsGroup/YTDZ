 ...
 */
Ext.define('yt.view.ytmap.timeline.TimeLine', {
    extend: 'Ext.DataView',
    xtype: 'timeline',
    ...
    requires: [
        'yt.view.ytmap.timeline.TimeLineController',
        'yt.view.ytmap.timeline.TimeLineModel'
    ],
    ...
    itemTpl: [
        ...
        '<img src="resources/images/yt/timeline/1.png" style="{rank:this.setImgBackgroudColor}">' +
        '</div>' +
        '<tpl>' +
        '<div class="contents-wrap">' +
        '<div class="shared-by">预警设备：<a href="#">{devicename}</a></div>' +
        '<div>预警时间：{alarmtime:this.elapsed}</div>' +
        '<div>预警内容：{content}</div>' +
        '</div>' +
        '</div>' +
        '</tpl>' +
        '</div>',
        {
            ...
            elapsed: function (value) {
                ...
                return Ext.Date.format(value, 'Y年m月d日 H:i:s');
            }
        }
    ]
});